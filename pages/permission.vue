<template>
  <the-portlet :title="$t('permission.list')">
    ...
    <data-table
      ...
      :server-side="true"
      :actions="actions"
      url="/admin/permissions/list-for-datatable"
      order-type="desc"
    />
...
import PermissionModal from '~/features/PermissionModal'
import { notifyDeleteSuccess, notifyTryAgain } from '~/utils/bootstrap-notify'
import { PERMISSION_CRUD } from '~/constants/permissions'
  ...
  methods: {
    ...
    deleteItem(table, rowData) {
      this.$bvModal
        ...
        .then(async (value) => {
          if (value) {
            await this.$axios.patch('/admin/permissions/' + rowData.id, {
              isDeleted: true
            })
