    ...
    rowsSelected(value) {
      if (this.selectable) {
        ...
        $(table)
          ...
          .html(`${this.$t('selected')} ${value.length} ${this.$t('record')}`)
      }
    },
    columns: {
      handler() {
  ...
  methods: {
    initTable(reInit = false) {
      ...
      const defaultConfigs = {
        ...
        language: {
          aria: {
            sortAscending: this.$t('sort_up_ascending'),
            sortDescending: this.$t('sort_reduction')
          },
          processing:
            '<div><span role="status" class="spinner-border text-primary mb-2"><span class="sr-only">Spinning</span></span></div><span>&nbsp;&nbsp;&nbsp; ' +
            this.$t('loading') +
            '...</span></div>',
          emptyTable: this.$t('no_data'),
          info: `_START_ - _END_ ${this.$t('of')} _TOTAL_ ${this.$t('record')}`,
          infoEmpty: '',
          infoFiltered: '',
          lengthMenu: `_MENU_ ${this.$t('record')}`,
          search: '',
          zeroRecords: this.$t('no_data'),
          paginate: {
          ...
          searchPlaceholder: this.$t('enter_search_keyword'),
          select: {
            rows: {
              _: '  %d rows selected',
              '0': '',
